...
exports.addAddress = async (req, res) => {
    ...
    try {
        ...
        customer.addresses.push({ street, ward, district, province, isDefault: isDefault || false });
        await customer.save();
        
        res.json({ success: true, message: 'Thêm địa chỉ thành công', addresses: customer.addresses });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.deleteAddress = async (req, res) => {
    try {
        ...
        await customer.save();
        res.json({ success: true, message: 'Xóa địa chỉ thành công', addresses: customer.addresses });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};